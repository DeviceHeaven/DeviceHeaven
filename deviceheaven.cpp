...
EthernetClient client; // inicializa cliente Ethernet
...
/* Descrição: Função utilizada para enviar dados para o DeviceHeaven com o ESP8266. */
void send_to_DH(String TOKEN, String LEITURA_ID, String LEITURA)
{
	 delay(1);
     String cmd = "AT+CIPSTART=4,\"TCP\",\""; //Configura conex�o TCP
...
/* Descrição: Função utilizada para enviar dados para o DeviceHeaven com o W5100. */
void send_to_DH2(String TOKEN, String LEITURA_ID, String LEITURA)
{
  ...
  if (client.connect(IP, 9012)) { // abre socket TCP 
...
void send_to_DH2(String TOKEN, String LEITURA_ID, String LEITURA)
{
  ...
  if (client.connect(IP, 9012)) { // abre socket TCP 
	...
	Serial.println("\nDADOS ENVIADOS COM SUCESSO PARA O DEVICE HEAVEN..");
	delay(1000);
	client.stop();                //fecha conexao TCP após envio
	Serial.println("Conexao fechada com sucesso\n");
	delay(1500);
    //Serial.println(cmd);
  } else 
        {
...
void Ethernet_start(uint8_t *mac)
{
      ...
      { 
       Serial.println("Conexao por DHCP bem sucedida");
	   delay(500);
	   Serial.println(Ethernet.localIP() );
	   }	
	   else 
	       {
			Serial.println("DHCP nao funcionou");
...
void Ethernet_start(uint8_t *mac, IPAddress ip)
{
     ...
     // { 
	  ...
	  // else 
	   //    {
			//Serial.println("Problema com o ponto de acesso");
	//	   }
